...
function handleHashChange() {
  ...
  const viewName = viewRenderers.has(requestedView)
    ? requestedView
    : "dashboard";
...
function setupEventListeners() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (navigation) {
      navigateTo(navigation.dataset.view);
      return;
    }
    const shortcut = event.target.closest("[data-navigate]");
    if (shortcut) {
      navigateTo(shortcut.dataset.navigate);
      return;
    }
    const tab = event.target.closest("[data-people-tab]");
    if (tab) {
      switchPeopleTab(tab.dataset.peopleTab);
      return;
    }
    if (event.target.closest("[data-save-hypothesis]")) {
      saveHypothesis();
      return;
    }
  ...
  document
    ...
    .addEventListener("change", sortEvidence);
  document
